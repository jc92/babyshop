...
async function ask(question: string, options?: { silent?: boolean }) {
  if (options?.silent) {
    const answer = await hide(rl.question.bind(rl))(question);
...
async function main() {
  ...
  const googleClientSecret = await ask("Enter GOOGLE_CLIENT_SECRET: ", { silent: true });
