...
import { defaultProfile, type PreferenceProfile } from "@/data/preferences";
import {
  categories,
  milestones,
  products,
...
export default function Home() {
  ...
  const budgetCopy: Record<PreferenceProfile["budget"], string> = {
        ...
        <section className="grid gap-6 rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
          <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
            <div>
              ...
              <p className="text-sm text-slate-600">
                Track what’s coming next from the third trimester through age 3. Filter by milestone to drill into details.
              </p>
            </div>
          ...
          <div className="grid gap-4 md:grid-cols-2">
            {timelineMilestones.map((milestone) => (
              <article
                key={milestone.id}
                className="relative overflow-hidden rounded-lg border border-slate-200 bg-slate-50 p-5 shadow-sm"
              >
                <div className="flex items-center justify-between">
                  ...
                  <div className="rounded-md bg-indigo-100 px-3 py-1 text-xs font-semibold text-indigo-700">
                    {products.filter((item) => item.milestoneIds.includes(milestone.id)).length} items
                  </div>
                </div>
              ...
              {categories.map((category) => {
                ...
                return (
                  <button
                    ...
                    className={`rounded-full border px-3 py-1 transition ${isActive ? "border-indigo-500 bg-indigo-500 text-white" : "border-slate-200 bg-white text-slate-600"}`}
                  >
                    {category.label}
                  </button>
                );
